...
void shell_ips_search_cb(struct bt_ips_search_rt *rt)
{
	...
	init_info.match_id = g_match_id; // åŒ¹é…ç , 4bytes éšæœºæ•°
	init_info.role = role;
	memcpy(init_info.name, TEST_IPS_NAME ,strlen(TEST_IPS_NAME));
	init_info.ch_max = rt->t_num; //æœ€å¤šå…è®¸æŽ¥æ”¶çš„éŸ³é¢‘é€šé“æ•°é‡
	init_info.tx_codec = BT_IPS_TX_CODEC_OPUS; //æœ¬æœºå¹¿æ’­çš„ç¼–ç æ ¼å¼
	init_info.comp_ratio = BT_IPS_COMP_RATIO_8; // ç¼–ç åŽ‹ç¼©æ¯”
	init_info.ch_mode = BT_IPS_TX_CH_SINGLE; //å•å£°é“æˆ–åŒå£°é“
	init_info.samp_freq = BT_IPS_SAMPLE_FREQ_8; //é‡‡æ ·çŽ‡8KHZ or 16KHZ
...
static int shell_cmd_ips_search(const struct shell *shell, size_t argc, char *argv[])
{
	...
	info.match_id = g_match_id; // åŒ¹é…ç , 4bytes éšæœºæ•°
	info.role = role;
...
static int shell_cmd_ips_start(const struct shell *shell, size_t argc, char *argv[])
{
	...
	init_info.role = role;
	memcpy(init_info.name, TEST_IPS_NAME ,strlen(TEST_IPS_NAME));
	init_info.ch_max = ch; //æœ€å¤šå…è®¸æŽ¥æ”¶çš„éŸ³é¢‘é€šé“æ•°é‡
	init_info.tx_codec = BT_IPS_TX_CODEC_OPUS; //æœ¬æœºå¹¿æ’­çš„ç¼–ç æ ¼å¼
	init_info.comp_ratio = BT_IPS_COMP_RATIO_8; // ç¼–ç åŽ‹ç¼©æ¯”
	init_info.ch_mode = BT_IPS_TX_CH_SINGLE; //å•å£°é“æˆ–åŒå£°é“
	init_info.samp_freq = BT_IPS_SAMPLE_FREQ_8; //é‡‡æ ·çŽ‡8KHZ or 16KHZ
