...
static int ota_backend_cwf_ioctl(struct ota_backend *backend, int cmd, unsigned int param)
{
	struct ota_backend_cwf *backend_cwf = CONTAINER_OF(backend,
		struct ota_backend_cwf, backend);
	struct svc_prot_context *svc_ctx = &backend_cwf->svc_ctx;
	int err;
...
static int ota_backend_cwf_read(struct ota_backend *backend, int offset, uint8_t *buf, int size)
{
	struct ota_backend_cwf *backend_cwf = CONTAINER_OF(backend,
		struct ota_backend_cwf, backend);
	struct svc_prot_context *svc_ctx = &backend_cwf->svc_ctx;
...
static int ota_backend_cwf_read_prepare(struct ota_backend *backend, int offset, uint8_t *buf, int size)
{
	struct ota_backend_cwf *backend_cwf = CONTAINER_OF(backend,
		struct ota_backend_cwf, backend);
	struct svc_prot_context *svc_ctx = &backend_cwf->svc_ctx;
...
static int ota_backend_cwf_read_complete(struct ota_backend *backend, int offset, uint8_t *buf, int size)
{
	struct ota_backend_cwf *backend_cwf = CONTAINER_OF(backend,
		struct ota_backend_cwf, backend);
	struct svc_prot_context *svc_ctx = &backend_cwf->svc_ctx;
...
static int ota_backend_cwf_open(struct ota_backend *backend)
{
	struct ota_backend_cwf *backend_cwf = CONTAINER_OF(backend,
		struct ota_backend_cwf, backend);
	struct svc_prot_context *svc_ctx = &backend_cwf->svc_ctx;
...
static int ota_backend_cwf_close(struct ota_backend *backend)
{
	struct ota_backend_cwf *backend_cwf = CONTAINER_OF(backend,
		struct ota_backend_cwf, backend);
	struct svc_prot_context *svc_ctx = &backend_cwf->svc_ctx;
...
void ota_backend_cwf_exit(struct ota_backend *backend)
{
	...
	if (backend_cwf->svc_ctx.send_buf)
		mem_free(backend_cwf->svc_ctx.send_buf);

	mem_free(backend_cwf);
}
...
static void sppble_connect_cb(int connected, uint8_t connect_type)
{
	...
	/* avoid connect again after exit */
	if (g_backend_cwf) {
		backend = &g_backend_cwf->backend;
		if (backend->cb) {
			SYS_LOG_INF("call cb %p", backend->cb);
			if (connected && (SPP_CONNECT_TYPE == connect_type)) {
				g_backend_cwf->svc_ctx.ota_unit_size = OTA_UNIT_FRCOMM_SIZE;
			} else if (connected && (BLE_CONNECT_TYPE == connect_type)) {
				mtu = bt_manager_get_ble_mtu();
				if ((mtu > OTA_UNIT_BLE_SIZE) || (mtu < 100)) {
					g_backend_cwf->svc_ctx.ota_unit_size = OTA_UNIT_BLE_SIZE;
				} else {
					g_backend_cwf->svc_ctx.ota_unit_size = mtu - 15;
				}
#ifdef CONFIG_OTA_GATT_OVER_EDR_TEST
				#define OTA_UNIT_EDR_SIZE		(323)
				if (!bt_manager_ble_is_connected()) {
					g_backend_cwf->svc_ctx.ota_unit_size = OTA_UNIT_EDR_SIZE;
				}
#endif
			} else {
				g_backend_cwf->svc_ctx.ota_unit_size = OTA_UNIT_SIZE;
			}
...
#endif
			if (!connected) {
				g_backend_cwf->svc_ctx.connect_type = NONE_CONNECT_TYPE;
			} else {
				g_backend_cwf->svc_ctx.connect_type = connect_type;
			}
...
static struct ota_backend_api ota_backend_api_cwf = {
	/* .init = ota_backend_cwf_init, */
	.exit = ota_backend_cwf_exit,
	.open = ota_backend_cwf_open,
	.close = ota_backend_cwf_close,
	.read = ota_backend_cwf_read,
	.ioctl = ota_backend_cwf_ioctl,
	.read_prepare = ota_backend_cwf_read_prepare,
	.read_complete = ota_backend_cwf_read_complete,
};

struct ota_backend *ota_backend_cwf_init(ota_backend_notify_cb_t cb,
					struct ota_backend_cwf_init_param *param)
{
	struct ota_backend_cwf *backend_cwf;
	struct svc_prot_context *svc_ctx;
	struct sppble_stream_init_param init_param;

	SYS_LOG_INF("init");

	backend_cwf = mem_malloc(sizeof(struct ota_backend_cwf));
	if (!backend_cwf) {
		SYS_LOG_ERR("malloc failed");
		return NULL;
	}

	memset(backend_cwf, 0x0, sizeof(struct ota_backend_cwf));
	svc_ctx = &backend_cwf->svc_ctx;
	...
	g_backend_cwf = backend_cwf;

	ota_backend_init(&backend_cwf->backend, OTA_BACKEND_TYPE_BLUETOOTH,
			 &ota_backend_api_cwf, cb);
	...
	return &backend_cwf->backend;
}

io_stream_t ota_backend_cwf_get_stream(struct ota_backend *backend)
{
	return (g_backend_cwf ?  g_backend_cwf->svc_ctx.sppble_stream : NULL);
}
