...
int usbh_audio_open(struct usbh_audio *audio_class, const char *name, struct usbh_audio_config *config)
{
    ...
    for (uint8_t i = 0; i < audio_class->module_num; i++) {
        if (strcmp(name, audio_class->module[i].name) == 0) {
            for (uint8_t j = 0; j < audio_class->num_of_intf_altsettings; j++) {
                if (audio_class->module[i].altsetting[j].bitresolution != config->bitresolution || \
                    audio_class->module[i].altsetting[j].channels != config->channels) {
                        continue;
                    }
                for (uint8_t k = 0; k < audio_class->module[i].altsetting[j].sampfreq_num; k++) {
                    if (audio_class->module[i].altsetting[j].sampfreq[k] == config->samp_freq) {
...
freq_found:
    ...
    if (audio_class->module[module_index].is_opened) {
        return -USB_ERR_BUSY;
    }
    ...
    memcpy(g_audio_buf, &config->samp_freq, 3);
    ...
    audio_class->module[module_index].is_opened = true;
    return ret;
}
...
int usbh_audio_close(struct usbh_audio *audio_class, const char *name)
{
    ...
    audio_class->module[module_index].is_opened = false;
