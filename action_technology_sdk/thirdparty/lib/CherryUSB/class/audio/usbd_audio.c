...
static usb_audio_private_cmd_cb usbd_audio_private_cmd;
...
static int audio_class_interface_request_handler(uint8_t busid, struct usb_setup_packet *setup, uint8_t **data, uint32_t *len)
{
	...
	/* implement undefine cmd Support: process Actions pc tool switch fixed command data */
	if((control_selector==AUDIO_CONTROL_MUTE) && (subtype==AUDIO_CONTROL_UNDEF)) {
		USB_LOG_INFO("non-standerd cmd, entity_id: 0x%02x, subtype: 0x%02x\r\n", entity_id, subtype);
		if (*len==LEN_PC_TOOL_SWITCH && usbd_audio_private_cmd) {
			usbd_audio_private_cmd(UAC_PC_TOOL_SWITCH_CMD, len, pri_data_ptr);
		}
                    ...
                    if (g_usbd_audio[busid].uac_version < 0x0200) {
                        switch (setup->bRequest) {
                            ...
                            case AUDIO_REQUEST_GET_MIN:
								...
								(*data)[0] = LOW_BYTE(MINIMUM_VOLUME);
								(*data)[1] = HIGH_BYTE(MINIMUM_VOLUME);
#endif
                                *len = 2;
                                break;
                            case AUDIO_REQUEST_GET_MAX:
                            ...
                            case AUDIO_REQUEST_GET_RES:
                                ...
                                (*data)[1] = 0x01;
#else
								(*data)[0] = LOW_BYTE(RESOTION_VOLUME); /* -60 dB */
								(*data)[1] = HIGH_BYTE(RESOTION_VOLUME);
#endif
                                *len = 2;
                                break;
                            default:
    ...
    switch (event) {
        ...
        case USBD_EVENT_SET_INTERFACE: {
            ...
            if (intf->bAlternateSetting) {
                usbd_audio_open(busid, intf->bInterfaceNumber, intf->bAlternateSetting);
            } else {
