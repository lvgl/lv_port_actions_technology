...
#if defined(CONFIG_USBDEV_MSC_DOUBLE_BUF_STRUCT)
/* Used to represent a buffer in USB MSC */
enum USB_Buffer_Index {
    USB_BUF_FIRST = 0,  /* The first buf */
    USB_BUF_SECOND = 1  /* The second buf */
};

enum USB_Buffer_Dir {
    USB_DATA_IDLE = 0,  /* Ignore operation */
    USB_DATA_IN,  		/* Write data */
    USB_DATA_OUT,  		/* Read data */
};
...
USB_NOCACHE_RAM_SECTION struct usbd_msc_priv {
    ...
    uint32_t nbytes;
#if defined(CONFIG_USBDEV_MSC_DOUBLE_BUF_STRUCT)
	USB_MEM_ALIGNX uint8_t block_buffer_alternate[CONFIG_USBDEV_MSC_MAX_BUFSIZE];	/* Second transfer buffer */
	uint8_t buf_index;								/* Used for ping-pong buf index */

	uint8_t forecast_blun;
	uint32_t forecast_start_sector;
	uint32_t forecast_transfer_len;
	uint8_t forecast_direction;
#endif
#elif defined(CONFIG_USBDEV_MSC_POLLING)
...
#ifdef CONFIG_USBDEV_MSC_DOUBLE_BUF_STRUCT
void usbdev_msc_scsi_handle(uint8_t busid);
#endif
#endif
...
void msc_storage_notify_handler(uint8_t busid, uint8_t event, void *arg)
{
    ...
    switch (event) {
        case USBD_EVENT_INIT:
            ...
            g_usbd_msc[busid].usbd_msc_thread = usb_osal_thread_create("usbd_msc", CONFIG_USBDEV_MSC_STACKSIZE, CONFIG_USBDEV_MSC_PRIO, usbdev_msc_thread, (void *)(size_t)busid);
            if (g_usbd_msc[busid].usbd_msc_thread == NULL) {
                USB_LOG_ERR("No memory to alloc for g_usbd_msc[busid].usbd_msc_thread\r\n");
            }
#if defined(CONFIG_USBDEV_MSC_DOUBLE_BUF_STRUCT)
			g_usbd_msc[busid].buf_index = USB_BUF_FIRST;
			g_usbd_msc[busid].forecast_direction = USB_DATA_IDLE;
			g_usbd_msc[busid].forecast_blun = 0;
			g_usbd_msc[busid].forecast_start_sector = 0;
			g_usbd_msc[busid].forecast_transfer_len = 0;
#endif
#elif defined(CONFIG_USBDEV_MSC_POLLING)
...
static bool SCSI_requestSense(uint8_t busid, uint8_t **data, uint32_t *len)
{
    ...
    if (g_usbd_msc[busid].disk_is_ready) {
...
static bool SCSI_write10(uint8_t busid, uint8_t **data, uint32_t *len)
{
	...
	if (g_usbd_msc[busid].buf_index == USB_BUF_FIRST) {
		cur_buffer = g_usbd_msc[busid].block_buffer;
	} else if (g_usbd_msc[busid].buf_index == USB_BUF_SECOND) {
		cur_buffer = g_usbd_msc[busid].block_buffer_alternate;
	} else {
	}
#else
	cur_buffer = g_usbd_msc[busid].block_buffer;
    ...
    usbd_ep_start_read(busid, mass_ep_data[busid][MSD_OUT_EP_IDX].ep_addr, cur_buffer, data_len);
#ifdef CONFIG_USBDEV_MSC_DOUBLE_BUF_STRUCT
	if (g_usbd_msc[busid].buf_index == USB_BUF_FIRST) {
		g_usbd_msc[busid].buf_index = USB_BUF_SECOND;
	} else if (g_usbd_msc[busid].buf_index == USB_BUF_SECOND) {
		g_usbd_msc[busid].buf_index = USB_BUF_FIRST;
	} else {
	}
...
static bool SCSI_processRead(uint8_t busid)
{
	...
	if ((g_usbd_msc[busid].forecast_blun == g_usbd_msc[busid].cbw.bLUN) &&				\
		(g_usbd_msc[busid].forecast_start_sector == g_usbd_msc[busid].start_sector) &&	\
		(g_usbd_msc[busid].forecast_transfer_len == transfer_len) && 					\
		(g_usbd_msc[busid].forecast_transfer_len != 0)) {
	} else
#endif
	{
	#ifndef CONFIG_USBDEV_MSC_DOUBLE_BUF_STRUCT
		cur_buffer = g_usbd_msc[busid].block_buffer;
	#endif
	    if (usbd_msc_sector_read(busid, g_usbd_msc[busid].cbw.bLUN, g_usbd_msc[busid].start_sector, cur_buffer, transfer_len) != 0) {
    ...
    usbd_ep_start_write(busid, mass_ep_data[busid][MSD_IN_EP_IDX].ep_addr, cur_buffer, transfer_len);
	...
	if (g_usbd_msc[busid].stage != MSC_SEND_CSW) {
		g_usbd_msc[busid].forecast_blun = g_usbd_msc[busid].cbw.bLUN;
		g_usbd_msc[busid].forecast_start_sector = g_usbd_msc[busid].start_sector;
		g_usbd_msc[busid].forecast_transfer_len = transfer_len;
		g_usbd_msc[busid].forecast_direction = USB_DATA_IN;
		usbdev_msc_scsi_handle(busid);
	} else {
		g_usbd_msc[busid].forecast_blun = 0;
		g_usbd_msc[busid].forecast_start_sector = 0;
		g_usbd_msc[busid].forecast_transfer_len = 0;
...
static bool  SCSI_processWrite(uint8_t busid, uint32_t nbytes)
{
	...
	#ifndef CONFIG_USBDEV_MSC_DOUBLE_BUF_STRUCT
		cur_buffer = g_usbd_msc[busid].block_buffer;
		if (usbd_msc_sector_write(busid, g_usbd_msc[busid].cbw.bLUN, g_usbd_msc[busid].start_sector, cur_buffer, nbytes) != 0) {
	...
	g_usbd_msc[busid].forecast_blun = g_usbd_msc[busid].cbw.bLUN;
	g_usbd_msc[busid].forecast_start_sector = g_usbd_msc[busid].start_sector;
	g_usbd_msc[busid].forecast_transfer_len = nbytes;
#else
	...
	if (g_usbd_msc[busid].nsectors == 0) {
		...
		g_usbd_msc[busid].forecast_direction = USB_DATA_OUT;
		usbdev_msc_scsi_handle(busid);
	#endif
	}
...
void usbdev_msc_scsi_handle(uint8_t busid)
{
    ...
    if (g_usbd_msc[busid].forecast_direction == USB_DATA_OUT) {
		if (usbd_msc_sector_write(busid, g_usbd_msc[busid].forecast_blun, g_usbd_msc[busid].forecast_start_sector, cur_buffer, g_usbd_msc[busid].forecast_transfer_len) != 0) {
			SCSI_SetSenseData(busid, SCSI_KCQHE_WRITEFAULT);
		}

		g_usbd_msc[busid].start_sector += (g_usbd_msc[busid].forecast_transfer_len / g_usbd_msc[busid].scsi_blk_size[g_usbd_msc[busid].cbw.bLUN]);
		g_usbd_msc[busid].forecast_start_sector = g_usbd_msc[busid].start_sector;
    } else if (g_usbd_msc[busid].forecast_direction == USB_DATA_IN) {
		if (usbd_msc_sector_read(busid, g_usbd_msc[busid].forecast_blun, g_usbd_msc[busid].forecast_start_sector, cur_buffer, g_usbd_msc[busid].forecast_transfer_len) != 0) {
			SCSI_SetSenseData(busid, SCSI_KCQHE_UREINRESERVEDAREA);
		}
    } else {
    }
...
void usbd_msc_disk_ready(uint8_t busid, bool is_ready)
{
    g_usbd_msc[busid].disk_is_ready = is_ready;
}
