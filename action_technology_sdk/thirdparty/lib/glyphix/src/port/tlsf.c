...
#if !TLSF_DEBUG_ENABLE
    tlsf_assert(!block_is_last(block));
#else
    if(block_is_last(block))
    {
        //rt_kprintf("========= block = 0x%x ==========\n", block);
        tlsf_assert(0 && "block_is_last");
    }
#endif
...
static void block_trim_free(control_t *control, block_header_t *block, size_t size)
{
    ...
    {
        //rt_kprintf("========= block = 0x%x ==========\n", block);
        tlsf_assert(0 && "block must be free");
    }
#endif
...
static block_header_t *block_locate_free(control_t *control, size_t size)
{
    ...
    if (block)
    {
        ...
        {
            //rt_kprintf("======= control = 0x%x block = 0x%x ==============\n", control, block);
            tlsf_assert(0 && "block_size");
        }
#endif
        remove_free_block(control, block, fl, sl);
    }
...
void tlsf_free(tlsf_t tlsf, void *ptr)
{
    ...
    if (ptr)
    {
        ...
        if (block_is_free(block))
        {
            //rt_kprintf("(tlsf_free)block is freed\r\n");
            //rt_kprintf("block size = %ld ptr = 0x%x block = 0x%x\r\n",block->size, ptr, block);
        }
