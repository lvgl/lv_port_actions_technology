...
} lv_draw_swrender_unit_t;

/**********************
 *  STATIC PROTOTYPES
 **********************/

static void _sw_blit(lv_draw_unit_t * draw_unit, const lv_draw_buf_t * src_buf,
...
{
    lv_draw_swrender_unit_t * draw_unit = lv_draw_create_unit(sizeof(*draw_unit));
    if (draw_unit) {
...
void lv_gpu_draw_buf_sw_clear(lv_draw_buf_t * draw_buf, const lv_area_t * area)
{
    ...
    uint32_t line_length = (width * bpp + 7) >> 3;
    ...
    if (dest_buf->header.cf == LV_COLOR_FORMAT_RGB565) {
        switch (src_buf->header.cf) {
            ...
            case LV_COLOR_FORMAT_A2:
                ...
                break;
            case LV_COLOR_FORMAT_A1:
                sw_blend_a1_over_rgb565(dest8, src_data, color32, dest_buf->header.stride,
                                        src_buf->header.stride, src_bofs, dest_w, dest_h);
                break;
            default:
        ...
        switch (src_buf->header.cf) {
            ...
            case LV_COLOR_FORMAT_A2:
                ...
                break;
            case LV_COLOR_FORMAT_A1:
                sw_blend_a1_over_rgb888(dest8, src_data, color32, dest_buf->header.stride,
                                        src_buf->header.stride, src_bofs, dest_w, dest_h);
                break;
            default:
                LV_LOG_ERROR("Unsupported src cf %x", src_buf->header.cf);
                          ...
                          const lv_draw_buf_t * src_buf, const lv_area_t * coords)
{
    ...
    if (transform_fn || transform_index_fn) {
        sw_matrix_t matrix;
        uint8_t *dest8 = (uint8_t *)lv_draw_buf_goto_xy(dest_buf,
                    draw_unit->clip_area->x1 - draw_unit->target_layer->buf_area.x1,
                    draw_unit->clip_area->y1 - draw_unit->target_layer->buf_area.y1);
                           ...
                           const lv_area_t * coords, bool no_cache)
{
    ...
    if (draw_dsc->tile) {
        ...
        while (tile_area.y1 <= draw_unit->clip_area->y2) {
            while (tile_area.x1 <= draw_unit->clip_area->x2) {
                ...
                if (lv_area_intersect(&clip_area, &tile_area, draw_unit->clip_area)) {
                    ...
                    _sw_blit(draw_unit, decoder_dsc.decoded,
        ...
        if (draw_dsc->rotation || draw_dsc->scale_x != LV_SCALE_NONE || draw_dsc->scale_y != LV_SCALE_NONE) {
            ...
            _sw_blit(draw_unit, decoder_dsc.decoded, &rel_img_area,
...
static void _sw_draw_glyph(lv_draw_unit_t * draw_unit, lv_draw_glyph_dsc_t * glyph_dsc)
{
    ...
    _sw_blit(draw_unit, glyph_dsc->glyph_data, &rel_coords,
...
static void _sw_draw_execute(lv_draw_swrender_unit_t * u)
{
...
static int32_t _sw_draw_dispatch(lv_draw_unit_t * draw_unit, lv_layer_t * layer)
{
    lv_draw_swrender_unit_t * u = (lv_draw_swrender_unit_t *)draw_unit;

    /* Return immediately if it's busy with draw task. */
    if (u->task_act) return 0;

    /* Try to get an ready to draw. */
    lv_draw_task_t * t = lv_draw_get_next_available_task(layer, NULL, SWRENDER_DRAW_UNIT_ID);

    /* Return 0 is no selection, some tasks can be supported by other units. */
    if (t == NULL || t->preferred_draw_unit_id != SWRENDER_DRAW_UNIT_ID) return LV_DRAW_UNIT_IDLE;
...
static int32_t _sw_draw_evaluate(lv_draw_unit_t * draw_unit, lv_draw_task_t * task)
{
    ...
    if (task->preference_score > SWRENDER_DRAW_PREFERENCE_SCORE) {
        task->preference_score = SWRENDER_DRAW_PREFERENCE_SCORE;
        task->preferred_draw_unit_id = SWRENDER_DRAW_UNIT_ID;
    }
