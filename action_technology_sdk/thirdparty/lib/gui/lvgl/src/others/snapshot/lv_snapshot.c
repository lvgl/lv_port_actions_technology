...
{
    lv_draw_buf_t draw_buf;
    lv_draw_buf_init(&draw_buf, 1, 1, cf, LV_STRIDE_AUTO, NULL, UINT32_MAX);
    if(lv_snapshot_reshape_draw_buf(obj, &draw_buf) != LV_RESULT_OK) {
        return 0;
    }

    return draw_buf.header.stride * draw_buf.header.h;
}
