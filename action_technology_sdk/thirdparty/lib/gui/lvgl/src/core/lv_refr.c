...
static void lv_inv_area_insert(lv_display_t * disp, const lv_area_t * area_p, const lv_area_t * scr_area);
#endif
...
#if LV_USE_STRACE_OBJ_DRAW
    LV_STRACE_STRING_U32X5(LV_STRACE_ID_OBJ_DRAW,
            obj->class_p->name ? obj->class_p->name : "unknown", (uintptr_t)obj,
            clip_coords_for_obj.x1, clip_coords_for_obj.y1, clip_coords_for_obj.x2, clip_coords_for_obj.y2);
#endif
...
#if LV_INV_AREAS_SORT_Y
    lv_inv_area_insert(disp, &com_area, &scr_area);
#else
...
void lv_display_refr_timer(lv_timer_t * tmr)
{
    ...
    LV_STRACE_U32(LV_STRACE_ID_REFR, (uint32_t)disp_refr);
...
refr_finish:
    ...
    LV_STRACE_CALL(LV_STRACE_ID_REFR);
    LV_PROFILER_END;
}
...
static void refr_area_part(lv_layer_t * layer)
{
    ...
    /*Notify the display driven rendering some area has started*/
    lv_display_send_event(disp_refr, LV_EVENT_RENDER_AREA_START, &disp_refr->refreshed_area);
    ...
    lv_display_send_event(disp_refr, LV_EVENT_RENDER_AREA_READY, &disp_refr->refreshed_area);
    LV_PROFILER_END;
}
...
static void wait_for_flushing(lv_display_t * disp)
{
    ...
    if(disp->flush_wait_cb) {
        if(disp->flushing) {
            LV_STRACE_VOID(LV_STRACE_ID_WAIT_FLUSH);
            disp->flush_wait_cb(disp);
            LV_STRACE_CALL(LV_STRACE_ID_WAIT_FLUSH);
        }
        disp->flushing = 0;
    }
    else {
...
static void lv_inv_area_insert(lv_display_t * disp, const lv_area_t * area_p, const lv_area_t * scr_area)
{
    ...
    if(join >= 0) {
        ...
        lv_area_join(area_joined, area_joined, area_p);
        ...
        for(i = join + 1; i < inv_p; i++) {
            ...
            lv_area_join(area_joined, area_joined, &disp->inv_areas[i]);
            disp->inv_p--;
        }
