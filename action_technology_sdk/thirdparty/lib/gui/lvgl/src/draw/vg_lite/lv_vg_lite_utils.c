...
const char * lv_vg_lite_feature_string(vg_lite_feature_t feature)
{
    switch(feature) {
            ...
            FEATURE_ENUM_TO_STRING(NV24_INPUT);
            ...
            FEATURE_ENUM_TO_STRING(DST_BUFLEN_ALIGNED);
...
bool lv_vg_lite_is_dest_cf_supported(lv_color_format_t cf)
{
    switch(cf) {
        ...
        case LV_COLOR_FORMAT_RGB565A8:
            return vg_lite_query_feature(gcFEATURE_BIT_VG_24BIT_PLANAR) ? true : false;
    ...
    bool tiled)
{
    ...
    /* Map to vglite physical address */
...
void lv_vg_lite_flush_now(struct _lv_draw_vg_lite_unit_t * u)
{
    LV_ASSERT_NULL(u);
    LV_PROFILER_BEGIN;
