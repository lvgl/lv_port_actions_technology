...
#define ACTS_DECODER_NAME    "ACTS_IMGDEC"
...
static lv_result_t _acts_decoder_info(lv_image_decoder_t * decoder, lv_image_decoder_dsc_t * dsc, lv_image_header_t * header);
static lv_result_t _acts_decoder_open(lv_image_decoder_t * decoder, lv_image_decoder_dsc_t * dsc);
static void _acts_decoder_close(lv_image_decoder_t * decoder, lv_image_decoder_dsc_t * dsc);
...
void lv_acts_decoder_init(void)
{
    ...
    if(decoder) {
        lv_image_decoder_set_info_cb(decoder, _acts_decoder_info);
        lv_image_decoder_set_open_cb(decoder, _acts_decoder_open);
        lv_image_decoder_set_close_cb(decoder, _acts_decoder_close);

        decoder->name = ACTS_DECODER_NAME;
    }
}
...
void lv_acts_decoder_deinit(void)
{
    ...
    while((decoder = lv_image_decoder_get_next(decoder)) != NULL) {
        if(decoder->info_cb == _acts_decoder_info) {
...
static lv_result_t _acts_decoder_open_variable(lv_image_decoder_t * decoder, lv_image_decoder_dsc_t * dsc)
{
...
static lv_result_t _acts_decoder_open_file(lv_image_decoder_t * decoder, lv_image_decoder_dsc_t * dsc)
{
    LV_UNUSED(decoder); /*Unused*/
...
static lv_result_t _acts_decoder_open(lv_image_decoder_t * decoder, lv_image_decoder_dsc_t * dsc)
{
    ...
    switch(dsc->src_type) {
        case LV_IMAGE_SRC_VARIABLE:
            return _acts_decoder_open_variable(decoder, dsc);
        case LV_IMAGE_SRC_FILE:
            res = _acts_decoder_open_file(decoder, dsc);
            break;
        default:
            return LV_RESULT_INVALID;
    }
