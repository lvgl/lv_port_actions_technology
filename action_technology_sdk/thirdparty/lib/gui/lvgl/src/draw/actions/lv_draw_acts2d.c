...
#define ACTS2D_SCALE_MIN ((LV_SCALE_NONE + 14) / 15)
...
} lv_draw_acts2d_unit_t;

/**********************
 *  STATIC PROTOTYPES
 **********************/

static uint32_t _lv_cf_to_acts2d(lv_color_format_t cf, bool rbswap);
static uint32_t _lv_cf_to_acts2d_for_copy(lv_color_format_t cf);

static int _acts2d_fill(lv_draw_unit_t * draw_unit, const lv_area_t * clip_a, lv_color_t color, lv_opa_t opa);
static int _acts2d_rotate(lv_draw_unit_t * draw_unit, const lv_draw_image_dsc_t * draw_dsc,
                          const lv_draw_buf_t * src_buf, const lv_area_t * src_coords);
static int _acts2d_blit(lv_draw_unit_t * draw_unit, const lv_draw_buf_t * src_buf,
                        const lv_area_t * src_a, lv_color_t recolor, lv_opa_t opa);

static int32_t _acts2d_draw_dispatch(lv_draw_unit_t * draw_unit, lv_layer_t * layer);
static int32_t _acts2d_draw_dispatch_task(lv_draw_unit_t * draw_unit, lv_layer_t * layer, lv_draw_task_t * task);
static int32_t _acts2d_draw_evaluate(lv_draw_unit_t * draw_unit, lv_draw_task_t * task);
static int32_t _acts2d_draw_delete(lv_draw_unit_t * draw_unit);
static int32_t _acts2d_wait_for_finish(lv_draw_unit_t * draw_unit);
...
lv_draw_unit_t * lv_draw_acts2d_init(bool subunit)
{
    ...
    lv_draw_acts2d_unit_t * draw_unit;
    if(subunit) {
        draw_unit = lv_malloc_zeroed(sizeof(*draw_unit));
        draw_unit->base_unit.dispatch_task_cb = _acts2d_draw_dispatch_task;
    }
    else {
        draw_unit = lv_draw_create_unit(sizeof(*draw_unit));
        draw_unit->base_unit.dispatch_cb = _acts2d_draw_dispatch;
    }

    draw_unit->hdma2d = &g_hdma2d;
    draw_unit->base_unit.evaluate_cb = _acts2d_draw_evaluate;
    draw_unit->base_unit.delete_cb = _acts2d_draw_delete;
    draw_unit->base_unit.wait_for_finish_cb = _acts2d_wait_for_finish;
...
lv_result_t lv_draw_buf_acts2d_clear(lv_draw_buf_t * draw_buf, const lv_area_t * area)
{
    ...
    hdma2d->output_cfg.color_format = _lv_cf_to_acts2d_for_copy(header->cf);
...
static bool _acts2d_dest_cf_supported(lv_color_format_t cf)
{
    switch(cf) {
        case LV_COLOR_FORMAT_RGB565:
        case LV_COLOR_FORMAT_ARGB8565:
...
static bool _acts2d_src_cf_supported(lv_color_format_t cf)
{
    switch(cf) {
        case LV_COLOR_FORMAT_RGB565:
        case LV_COLOR_FORMAT_ARGB8565:
...
static bool _acts2d_src_cf_rotation_supported(lv_color_format_t cf)
{
    switch(cf) {
        case LV_COLOR_FORMAT_RGB565:
        case LV_COLOR_FORMAT_ARGB8888:
...
static int _acts2d_fill(lv_draw_unit_t * draw_unit, const lv_area_t * clip_a, lv_color_t color, lv_opa_t opa)
{
    hal_dma2d_handle_t * hdma2d = ((lv_draw_acts2d_unit_t *)draw_unit)->hdma2d;
    ...
    hdma2d->output_cfg.color_format = _lv_cf_to_acts2d(dest_buf->header.cf, rbswap);
    ...
    if(res < 0) {
        LV_LOG_WARN("dest 0x%x, w %u, h %u, stride %u, fmt 0x%x",
                          ...
                          const lv_draw_buf_t * src_buf, const lv_area_t * src_coords)
{
    hal_dma2d_handle_t * hdma2d = ((lv_draw_acts2d_unit_t *)draw_unit)->hdma2d;
    lv_draw_buf_t * dest_buf = draw_unit->target_layer->draw_buf;
    const lv_area_t * clip_a = draw_unit->clip_area;
    lv_area_t dest_area = {
        clip_a->x1 - draw_unit->target_layer->buf_area.x1, clip_a->y1 - draw_unit->target_layer->buf_area.y1,
        clip_a->x2 - draw_unit->target_layer->buf_area.x1, clip_a->y2 - draw_unit->target_layer->buf_area.y1,
    };
    ...
    hdma2d->output_cfg.color_format = _lv_cf_to_acts2d(dest_buf->header.cf, false);
    ...
    hdma2d->layer_cfg[1].color_format = _lv_cf_to_acts2d(src_buf->header.cf, false);
                        ...
                        const lv_area_t * src_a, lv_color_t recolor, lv_opa_t opa)
{
    ...
    lv_area_t dest_area = {
        ...
        draw_unit->clip_area->x2 - draw_unit->target_layer->buf_area.x1,
        draw_unit->clip_area->y2 - draw_unit->target_layer->buf_area.y1,
    };
    ...
    hdma2d->layer_cfg[1].color_format = _lv_cf_to_acts2d(src_buf->header.cf, rbswap);
    hdma2d->layer_cfg[1].input_pitch = src_buf->header.stride;
    hdma2d->layer_cfg[1].alpha_mode = HAL_DMA2D_COMBINE_ALPHA;
    ...
    if(res < 0) {
        LV_LOG_WARN("dest 0x%x, w %u, h %u, stride %u, fmt 0x%x; src 0x%x, w %u, h %u, stride %u, fmt 0x%x",
                dest_addr, dest_w, dest_h, dest_buf->header.stride, dest_buf->header.cf,
...
static void _acts2d_draw_wait_finish(lv_draw_unit_t * draw_unit)
{
    lv_draw_acts2d_unit_t * unit = (lv_draw_acts2d_unit_t *)draw_unit;
    hal_dma2d_poll_transfer(unit->hdma2d, -1);
}

static void _acts2d_draw_fill(lv_draw_unit_t * draw_unit,
              lv_draw_fill_dsc_t * dsc, const lv_area_t * coords)
{
    ...
    if(_acts2d_fill(draw_unit, &draw_area, dsc->color, dsc->opa) < 0)
...
static void _acts2d_draw_image(lv_draw_unit_t * draw_unit, const lv_draw_image_dsc_t * draw_dsc,
                              const lv_area_t * coords, bool no_cache)
{
    ...
    if(draw_dsc->tile) {
        ...
        while(tile_area.y1 <= tile_y_end) {
            while(tile_area.x1 <= tile_x_end) {
                ...
                if(lv_area_intersect(&clip_area, &tile_area, clip_area_ori)) {
                    ...
                    int res = _acts2d_blit(draw_unit, decoder_dsc.decoded,
                                &rel_img_area, draw_dsc->recolor, draw_dsc->opa);
                    if(res < 0) {
                        _acts2d_draw_wait_finish(draw_unit);
        ...
        if(draw_dsc->rotation || draw_dsc->scale_x != LV_SCALE_NONE || draw_dsc->scale_y != LV_SCALE_NONE) {
            ...
            lv_point_t pivot;
            pivot.x = (draw_dsc->pivot.x + coords->x1 - draw_area.x1) << 1;
            pivot.y = (draw_dsc->pivot.y + coords->y1 - draw_area.y1) << 1;

            lv_image_buf_get_transformed_area(&rel_img_area, w << 1, h << 1,
                    3600 - draw_dsc->rotation,
                    LV_SCALE_NONE * LV_SCALE_NONE / draw_dsc->scale_x,
                    LV_SCALE_NONE * LV_SCALE_NONE / draw_dsc->scale_y,
                    &pivot);
            rel_img_area.x1 = rel_img_area.x1 >> 1;
            rel_img_area.y1 = rel_img_area.y1 >> 1;
            rel_img_area.x2 = (rel_img_area.x2 + 1) >> 1;
            rel_img_area.y2 = (rel_img_area.y2 + 1) >> 1;
        ...
        if(draw_dsc->rotation) {
            _acts2d_rotate(draw_unit, draw_dsc, (void *)decoder_dsc.decoded, coords);
        }
        else {
            lv_area_move(&rel_img_area, -coords->x1, -coords->y1);
            int res = _acts2d_blit(draw_unit, (void *)decoder_dsc.decoded, &rel_img_area,
                            draw_dsc->recolor, draw_dsc->opa);
            if(res < 0) {
                _acts2d_draw_wait_finish(draw_unit);
                lv_draw_actsw_blit(draw_unit, (void *)decoder_dsc.decoded, &rel_img_area,
                            draw_dsc->recolor, draw_dsc->opa);
            }
        }
...
static void _acts2d_draw_layer(lv_draw_unit_t * draw_unit,
              const lv_draw_image_dsc_t * draw_dsc, const lv_area_t * coords)
{
    ...
    _acts2d_draw_image(draw_unit, &new_draw_dsc, coords, true);

    LV_PROFILER_END;
}

static void _acts2d_draw_glyph(lv_draw_unit_t * draw_unit, lv_draw_glyph_dsc_t * glyph_dsc)
{
    ...
    if(lv_area_get_width(&blend_area) >= 2) {
        ...
        res = _acts2d_blit(draw_unit, draw_buf, &rel_coords,
    ...
    if(res < 0) {
        lv_draw_actsw_blit(draw_unit, draw_buf, &rel_coords,
                glyph_dsc->color, glyph_dsc->opa);
    }
...
static void _acts2d_draw_letter_cb(lv_draw_unit_t * draw_unit,
                ...
                const lv_area_t * fill_area)
{
    if(glyph_dsc) {
        switch(glyph_dsc->format) {
            ...
            case LV_FONT_GLYPH_FORMAT_A8: {
                _acts2d_draw_glyph(draw_unit, glyph_dsc);
                break;
            }
            ...
            case LV_FONT_GLYPH_FORMAT_IMAGE: {
                ...
                _acts2d_draw_image(draw_unit, &img_dsc, glyph_dsc->letter_coords, true);
#endif
                break;
            }
    ...
    if(fill_dsc && fill_area) {
        _acts2d_draw_fill(draw_unit, fill_dsc, fill_area);
    }
}

static void _acts2d_draw_label(lv_draw_unit_t * draw_unit,
                const lv_draw_label_dsc_t * dsc, const lv_area_t * coords)
{
    if(dsc->opa <= LV_OPA_MIN) return;

    LV_PROFILER_BEGIN;
    lv_draw_label_iterate_characters(draw_unit, dsc, coords, _acts2d_draw_letter_cb);
    LV_PROFILER_END;
}

static void _acts2d_draw_execute(lv_draw_acts2d_unit_t * u)
{
    ...
    switch(task->type) {
        case LV_DRAW_TASK_TYPE_FILL:
            _acts2d_draw_fill(draw_unit, task->draw_dsc, &task->area);
            break;

        case LV_DRAW_TASK_TYPE_IMAGE:
            _acts2d_draw_image(draw_unit, task->draw_dsc, &task->area, false);
            break;

        case LV_DRAW_TASK_TYPE_LAYER:
            _acts2d_draw_layer(draw_unit, task->draw_dsc, &task->area);
            break;

        case LV_DRAW_TASK_TYPE_LABEL:
            _acts2d_draw_label(draw_unit, task->draw_dsc, &task->area);
            break;
...
static int32_t _acts2d_draw_dispatch(lv_draw_unit_t * draw_unit, lv_layer_t * layer)
{
    lv_draw_acts2d_unit_t * u = (lv_draw_acts2d_unit_t *)draw_unit;

    /* Return immediately if it's busy with draw task. */
    if(u->task_act) return 0;

    /* Try to get an ready to draw. */
    lv_draw_task_t * t = lv_draw_get_next_available_task(layer, NULL, ACTS2D_DRAW_UNIT_ID);

    /* Return 0 is no selection, some tasks can be supported by other units. */
    if(t == NULL || t->preferred_draw_unit_id != ACTS2D_DRAW_UNIT_ID) {
        _acts2d_draw_wait_finish(draw_unit);
...
static bool _acts2d_evaluate_task_layer(lv_draw_task_t * task)
{
    lv_draw_dsc_base_t * base_dsc = task->draw_dsc;
    lv_layer_t * layer = base_dsc->layer;

    lv_area_t area;
    if(lv_area_intersect(&area, &task->clip_area, &task->_real_area) == false)
        return false;

    if(lv_area_get_width(&area) < 2)
        return false;

    if(!_acts2d_dest_cf_supported(layer->color_format))
        return false;
...
static int32_t _acts2d_draw_evaluate(lv_draw_unit_t * draw_unit, lv_draw_task_t * task)
{
    ...
    if(hal_dma2d_get_state(u->hdma2d) == HAL_DMA2D_STATE_RESET)
        return 0;

    if(!_acts2d_evaluate_task_layer(task))
        return 0;
    ...
    switch(task->type) {
        ...
        case LV_DRAW_TASK_TYPE_LAYER: {
            ...
            if(draw_dsc->rotation || draw_dsc->scale_x < ACTS2D_SCALE_MIN || draw_dsc->scale_y < ACTS2D_SCALE_MIN)
                return 0;
            if(draw_dsc->scale_x != LV_SCALE_NONE || draw_dsc->scale_y != LV_SCALE_NONE) {
                if(!lv_area_is_in(&task->_real_area, &task->clip_area, 0))
                    return 0;
            }
#else
            if(draw_dsc->scale_x != LV_SCALE_NONE || draw_dsc->scale_y != LV_SCALE_NONE) {
                if(draw_dsc->scale_x < ACTS2D_SCALE_MIN || draw_dsc->scale_y < ACTS2D_SCALE_MIN)
                    return 0;
                if(draw_dsc->rotation && (draw_dsc->scale_x < LV_SCALE_NONE || draw_dsc->scale_y < LV_SCALE_NONE))
                    return 0;
            }
            ...
            if(task->type == LV_DRAW_TASK_TYPE_IMAGE) {
                if((draw_dsc->header.flags & (LV_IMAGE_FLAGS_COMPRESSED | LV_IMAGE_FLAGS_PREMULTIPLIED)) ||
                    _acts2d_src_cf_supported(draw_dsc->header.cf) == false) {
                    return 0;
                }

                if(draw_dsc->rotation && (layer->color_format == LV_COLOR_FORMAT_RGB888 ||
                         _acts2d_src_cf_rotation_supported(draw_dsc->header.cf) == false)) {
                    return 0;
                }
                ...
                lv_layer_t * layer_to_draw = (lv_layer_t *)draw_dsc->src;
                if(_acts2d_src_cf_supported(layer_to_draw->color_format) == false) {
                    return 0;
                }

                if(draw_dsc->rotation && (layer->color_format == LV_COLOR_FORMAT_RGB888 ||
                         _acts2d_src_cf_rotation_supported(layer_to_draw->color_format) == false)) {
                    return 0;
                }
            }
    ...
    if(task->preference_score > ACTS2D_DRAW_PREFERENCE_SCORE) {
        task->preference_score = ACTS2D_DRAW_PREFERENCE_SCORE;
        task->preferred_draw_unit_id = ACTS2D_DRAW_UNIT_ID;
    }
