...
LOG_MODULE_REGISTER(aeshw, CONFIG_LOG_DEFAULT_LEVEL);
...
struct aes_dev_s
{
	...
	unsigned int  aes_key_size;    /* AES KEY size */
...
static int aes_encrypt_by_cpu(void *out, const void *in, size_t size)
{
	size_t size_remain;
	size_t aes_len;
...
static int aes_encrypt_by_dma(void *out, const void *in, size_t size)
{
	uint32_t aes_inoutfifictl_val;
	uint32_t aes_ctrl_val;
...
static int aes_cypher_inner(void *out, const void *in, size_t size,
               unsigned char *iv, const void *key, size_t keysize,
               int mode, int encrypt)
{
	int ret = 0;
...
{
	extern int mbedtls_aes_setkey_enc_sw(mbedtls_aes_context *ctx,
	...
	g_aesdev.aes_key_size = keybits / 8;
	memcpy(g_aesdev.aes_key, key, g_aesdev.aes_key_size);

	/* needed in software XTS to set RK */
	mbedtls_aes_setkey_enc_sw(ctx, key, keybits);
...
int mbedtls_aes_setkey_dec(mbedtls_aes_context *ctx, const unsigned char *key,
                           unsigned int keybits)
{
	extern int mbedtls_aes_setkey_dec_sw(mbedtls_aes_context *ctx,
                                         const unsigned char *key,
                                         unsigned int keybits);
	...
	/* needed in software XTS to set RK */
	mbedtls_aes_setkey_dec_sw(ctx, key, keybits);
...
{
	LOG_DBG("HW-ECB length=16, mode=%d", mode);

	return aes_cypher_inner(output, input, 16,
                      NULL, g_aesdev.aes_key, g_aesdev.aes_key_size,
                      AES_MODE_ECB, mode);
}
                          ...
                          unsigned char *output)
{
	...
	LOG_DBG("HW-CBC length=%d, mode=%d", length, mode);

	if ((input == NULL) || (output == NULL) || (iv == NULL) || (length == 0)) {
		LOG_ERR("Input NULL err");
		return ret;
	}

	if (length % 16) {
		LOG_ERR("Hareware only supports length = N*16 bytes");
		return ret;
	}

	if (mode == MBEDTLS_AES_DECRYPT) {
		for (i = 0; i < 16; i++) {
			iv_temp[15 - i] = input[length - 1 - i];
		}
	}

	ret = aes_cypher_inner(output, input, length,
                     iv, g_aesdev.aes_key, g_aesdev.aes_key_size,
                     AES_MODE_CBC, mode);

	if (mode == MBEDTLS_AES_ENCRYPT) {
		for (i = 0; i < 16; i++) {
			iv[15 - i] = output[length - 1 - i];
		}
	} else if (mode == MBEDTLS_AES_DECRYPT) {
		for (i = 0; i < 16; i++) {
			iv[15 - i] = iv_temp[length - 1 - i];
		}
	}
                          ...
                          unsigned char *output)
{
	...
	LOG_DBG("HW-CTR length=%d", length);

	ret = aes_cypher_inner(output, input, length,
                      nonce_counter, g_aesdev.aes_key, g_aesdev.aes_key_size,
                      AES_MODE_CTR, MBEDTLS_AES_ENCRYPT);
