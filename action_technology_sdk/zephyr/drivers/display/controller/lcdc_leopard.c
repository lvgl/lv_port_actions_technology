...
#define USE_QSYNC_SDMA_RELOAD 1
...
static int lcdc_config_port(const struct device *dev, const struct display_videoport *port)
{
	...
	lcd_ctl = LCD_EN | LCD_CLK_EN | LCD_FIFO_EN | LCD_SDCHK_CLK_EN |
...
#else
	lcd_ctl = LCD_EN | LCD_CLK_EN | LCD_FIFO_EN |
			LCD_IN_RGB666_CVT_LOW | LCD_IN_RGB565_CVT_LOW /*| LCD_OUT_RGB565_FORCE_555*/;
#endif /* USE_LCDC_SEND_DBG */

	if (port->type != DISPLAY_PORT_QSPI_SYNC) {
		lcd_ctl |= LCD_HOLD_EN | LCD_AUTO_RST_FIFO_EN;
	} else {
#if USE_QSYNC_SDMA_RELOAD == 0
		lcd_ctl |= LCD_AUTO_RST_FIFO_EN;
#endif
	}
...
static int lcdc_config_mode(const struct device *dev, const struct display_videomode *mode)
{
	...
	if (mode->hactive > 1024 || mode->vactive > 1024 ||
	...
	if (lcdc_data->port->major_type == DISPLAY_PORT_SPI) {
		...
		if (lcdc_data->port->minor_type < DISPLAY_QSPI) {
			...
			LCDC->SPI_CTL = (LCDC->SPI_CTL & ~(SPI_AHB_DATA_MASK | LCD_SPI_DUAL_LANE_MASK)) |
							(LCD_SPI_SRC_SEL_AHB | LCD_SPI_AHB_CFG_DATA | LCD_SPI_RWL(4));
			for (; len >= 4; len -= 4) {
			...
			if (len > 0) {
				LCDC->SPI_CTL = (LCDC->SPI_CTL & ~LCD_SPI_RWL_MASK) | LCD_SPI_RWL(len);
				uint32_t data = LCDC->DATA;
				_read_config_data32(buf, &data, len);
			}
		...
		const struct display_buffer_descriptor *desc, const void *buf)
{
	...
	total_bytes = (uint32_t)copy_per_line * desc->height;
	if (total_bytes >= 0x100000) {
		LOG_ERR("exceed SDMA bytes 0x%x", total_bytes);
		return -EINVAL;
	}
