...
#ifndef CONFIG_DISPLAY_ENGINE_CLOCK_KHZ
#  if defined(CONFIG_DISPLAY_ENGINE_LARK)
#    define CONFIG_DISPLAY_ENGINE_CLOCK_KHZ 100000
#  else
#    define CONFIG_DISPLAY_ENGINE_CLOCK_KHZ 200000
#  endif
#endif /* CONFIG_DISPLAY_ENGINE_CLOCK_KHZ */

void nor_xip_set_locked(bool locked)
{
#if defined(CONFIG_SPI_FLASH_ACTS) && !defined(CONFIG_DISPLAY_ENGINE_LARK)
	if (locked) {
		spi0_nor_xip_lock();
	} else {
		spi0_nor_xip_unlock();
