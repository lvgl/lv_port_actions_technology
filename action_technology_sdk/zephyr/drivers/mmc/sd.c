...
#define CONFIG_MMC_SDCARD_LOW_POWER      1 /* If 1 to enable SD card low power */
...
static int mmc_check_stop_trans(struct sd_card_data *sd)
{
	int ret;
	uint32_t status = 0;
	...
	if (ret == 0) {
		...
		if ((status == R1_STATE_DATA) || (status == R1_STATE_RCV)) {
			LOG_DBG("stop trans, status:0x%x", status);
...
static int mmc_enter_low_power(struct sd_card_data *sd)
{
	...
	if (sd->on_high_performance) {
		err = mmc_check_stop_trans(sd);
		if (err) {
			LOG_ERR("%s, card busy err=%d", __func__, err);
			return err;
		}
	}
	...
	if (err == 0) {
		LOG_INF("mmc enter sleep success");
	}
#endif
	return err;
}
...
static int mmc_exit_low_power(struct sd_card_data *sd)
{
	...
	if (err == 0) {
		LOG_INF("mmc exit sleep success");
	}
			 ...
			 void *dst, int blk_size, int blk_num)
{
	...
	if (is_write) {
		...
		cmd->flags = MMC_RSP_R1 | MMC_CMD_ADTC | MMC_DATA_READ;
		/* suppose that flag is MMC_DATA_READ | MMC_LBE_DIS in high performance.
		 * update it to MMC_DATA_READ_DIRECT | MMC_LBE_DIS in condition of section >=2 data trans.
		 */
		if (sd->on_high_performance)
			cmd->flags |= MMC_LBE_DIS;
	}
	...
	if (sd->on_high_performance) {
		...
		if (sd->next_rw_offs == SD_CARD_INVALID_OFFSET) {
			...
			if ((sd->prev_is_write) && (is_write) && (addr == sd->next_rw_offs)) {
				...
				/* for section >=2 read trans, update flags to MMC_DATA_READ_DIRECT | MMC_LBE_DIS */
				cmd->flags = (MMC_RSP_R1 | MMC_CMD_ADTC | MMC_DATA_READ_DIRECT | MMC_LBE_DIS);
				sd->next_rw_offs += blk_num;
				LOG_DBG("%d, next_rw_offs 0x%x, blk_num %d", __LINE__, sd->next_rw_offs, blk_num);
			} else {
...
#if (CONFIG_MMC_SDCARD_LOW_POWER == 1)
	if (mmc_can_sleep(sd)) {
		k_work_cancel_delayable(&sd->timer);
...
#if (CONFIG_MMC_SDCARD_LOW_POWER == 1)
	if (mmc_can_sleep(sd)) {
		k_work_reschedule_for_queue(&sd->sd_workq,
	                                &sd->timer,
	                                K_MSEC(CONFIG_SD_IDLE_SLEEP_MS));
		sd->sleep_pending = true;
	}
#endif
...
static void sd_card_sleep_work(struct k_work *work);
...
static int sd_card_init(const struct device *dev)
{
	...
	k_work_init_delayable(&sd->timer, sd_card_sleep_work);
...
static void sd_card_sleep_work(struct k_work *work)
{
	struct sd_card_data *sd = CONTAINER_OF((struct k_work_delayable *)work,
	                                       struct sd_card_data,
	                                       timer);
	LOG_DBG("%s", __func__);
	sd_sem_take(&sd->lock, K_FOREVER);
	mmc_enter_low_power(sd);
	sd->sleep_pending = false;
	sd_sem_give(&sd->lock);
}
...
int sd_card_pm_control(const struct device *dev, enum pm_device_action action)
{
	...
	case PM_DEVICE_ACTION_SUSPEND:
		...
		/* if card is in the process of sleep, can not suspend */
		if (sd->sleep_pending)
			return -1;
		board_get_mmc0_pinmux_info(&pinmux_info);
		/* power off mmc card */
