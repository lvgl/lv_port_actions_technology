...
LOG_MODULE_REGISTER(DLELF, LOG_LEVEL_INF);
...
rt_err_t dlmodule_load_shared_object(struct rt_dlmodule* module, void *module_ptr)
{
    ...
    for (index = 0; index < elf_module->e_shnum; index ++)
    {
        ...
        for (i = 0; i < nr_reloc; i ++)
        {
            ...
            if ((sym->st_shndx != SHT_NULL) ||(ELF_ST_BIND(sym->st_info) == STB_LOCAL))
            {
                ...
                addr = (Elf_Addr)((rt_uint8_t *)module->mem_space + sym->st_value - vstart_addr);
    ...
    if (index != elf_module->e_shnum)
    {
        ...
        for (i = 0; i < shdr[index].sh_size / sizeof(Elf_Sym); i++)
        {
            ...
            {
                flag |= 0x01;
                priority = *(rt_uint16_t*)((rt_uint8_t *)module->mem_space + symtab[i].st_value - module->vstart_addr);
            ...
            {
                flag |= 0x02;
                stacksize = *(rt_uint32_t*)((rt_uint8_t *)module->mem_space + symtab[i].st_value - module->vstart_addr);
