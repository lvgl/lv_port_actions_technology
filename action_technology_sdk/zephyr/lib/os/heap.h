...
#ifndef __ASSERT_PUSH
#define __ASSERT_PUSH  __asm__ volatile("push {r0, r1, r2, r3}")
#endif
#ifndef __ASSERT_POP
#define __ASSERT_POP  __asm__ volatile("pop {r0, r1, r2, r3}")
#endif
...
struct z_heap_stats {
	chunksz_t used_chunks;
	chunksz_t peak_chunks;
};
...
static inline struct z_heap_stats* heap_get_stats(struct z_heap *h)
{
	chunk_unit_t *buf = chunk_buf(h);
	chunkid_t c = right_chunk(h, 0) - chunksz(sizeof(struct z_heap_stats));

	return (struct z_heap_stats*)&buf[c];
}
