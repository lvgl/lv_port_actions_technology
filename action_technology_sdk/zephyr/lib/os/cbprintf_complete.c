 ...
 */
__sleepfunc static size_t extract_decimal(const char **str)
{
	const char *sp = *str;
	size_t val = 0;
 ...
 */
__sleepfunc static void _ldiv5(uint64_t *v)
{
	uint32_t v_lo = *v;
	uint32_t v_hi = *v >> 32;
...
/* Division by 10 */
__sleepfunc static void _ldiv10(uint64_t *v)
{
	*v >>= 1;
	_ldiv5(v);
}
 ...
 */
__sleepfunc static char _get_digit(uint64_t *fr, int *digit_count)
{
	char rval;
 ...
 */
__sleepfunc static char *encode_uint(uint_value_type value,
 ...
 */
__sleepfunc static char *encode_float(double value,
			  struct conversion *conv,
			  int precision,
			  char *sign,
			  char *bps,
...
__sleepfunc static int outs(cbprintf_cb out,
		void *ctx,
		const char *sp,
		const char *ep)
{
...
__sleepfunc int cbvprintf(cbprintf_cb out, void *ctx, const char *fp, va_list ap)
{
