...
static struct {
	...
	/* cache buf */
	uint8_t data_cache_buf[FLASH_SEC_SIZE];
	uint32_t buf_offset;
...
{
	struct device *dev;
	printk("init coredump nand/sd storage\n");
#if  defined(CONFIG_BOARD_NANDBOOT) 
#define DUMP_FLASH_DEV_NAME CONFIG_SPINAND_FLASH_NAME
#else
#define DUMP_FLASH_DEV_NAME CONFIG_SD_NAME
#endif

	dev = (struct device *)device_get_binding(DUMP_FLASH_DEV_NAME);
	if (!dev) {
...
{
	int ret = 0;
	uint32_t off_addr = backend_ctx.base_addr+off;
	uint32_t byte_off, sec_off,cpy_size;
	uint8_t *ptr = dst;
	uint8_t *pc = backend_ctx.data_cache_buf;

	if (backend_ctx.storage_dev == NULL) {
		return -ENODEV;
	}
	byte_off = off_addr & (FLASH_SEC_SIZE-1);	
	if(byte_off){
		sec_off =  off_addr & (~(FLASH_SEC_SIZE-1));
		flash_read(backend_ctx.storage_dev, sec_off,  pc, FLASH_SEC_SIZE);
		cpy_size = FLASH_SEC_SIZE-byte_off;
		if(cpy_size > len)
			cpy_size = len;
		if(dst != NULL){		
			memcpy(ptr, pc+byte_off, cpy_size);
			ptr += cpy_size;
		}
		if (cb != NULL) {
			(*cb)(cb_arg, pc+byte_off, cpy_size);
		}
		sec_off += FLASH_SEC_SIZE;
		len -= cpy_size;
	}else{
		sec_off =  off_addr;
	}
	cpy_size = FLASH_SEC_SIZE;
	while (len > 0) {
		ret = flash_read(backend_ctx.storage_dev, sec_off, pc, FLASH_SEC_SIZE);
		if (ret != 0) {
			LOG_ERR("read off=0x%x error\n", sec_off);
			break;
		}
		...
		if (dst != NULL) {
			(void)memcpy(ptr, pc, cpy_size);
		}

		if (cb != NULL) {
			ret = (*cb)(cb_arg, pc, cpy_size);
			if (ret != 0) {
				break;
			}
		}
...
static int process_stored_dump(data_read_cb_t cb, void *cb_arg)
{
	...
	if ((hdr.id[0] != 'C') && (hdr.id[1] != 'D')) {
		ret = 0;
		LOG_ERR("veryfiy head err, 0x%x, 0x%x\n", hdr.id[0], hdr.id[1]);
		goto out;
	}
	...
	offset = ROUND_UP(sizeof(struct flash_hdr_t), FLASH_SEC_SIZE);
	ret = data_read(offset, NULL, hdr.size, cb, cb_arg);
	
	if (ret == 0) {		
		ret = (backend_ctx.checksum == hdr.checksum) ? 1 : 0;
		if(ret == 0){
			LOG_ERR("veryfiy checksum 0x%x!=0x%x\n", backend_ctx.checksum, hdr.checksum);
		}
	}
...
static void coredump_flash_backend_start(void)
{
	...
	backend_ctx.cur_offset = ROUND_UP(sizeof(struct flash_hdr_t), FLASH_SEC_SIZE);
	backend_ctx.w_size = 0;
	backend_ctx.error = 0;
	backend_ctx.buf_offset = 0;
	memset(backend_ctx.data_cache_buf, 0xff, FLASH_SEC_SIZE);
...
static void coredump_flash_backend_end(void)
{
	...
	//LOG_ERR("savelen=0x%x, off=0x%x\n", backend_ctx.w_size, backend_ctx.buf_offset);
	if(backend_ctx.buf_offset){ // flush all cache data
		backend_ctx.error = flash_write(backend_ctx.storage_dev,
					backend_ctx.base_addr+backend_ctx.cur_offset,
					backend_ctx.data_cache_buf, FLASH_SEC_SIZE);
		backend_ctx.cur_offset += FLASH_SEC_SIZE;		
		backend_ctx.w_size += backend_ctx.buf_offset;
		backend_ctx.buf_offset = 0;
	}
	
	/* Write header */
	hdr.size = backend_ctx.w_size;
	...
	memset(backend_ctx.data_cache_buf, 0xff, FLASH_SEC_SIZE);
	memcpy(backend_ctx.data_cache_buf, (void *)&hdr, sizeof(hdr));
	ret = flash_write(flash_dev, backend_ctx.base_addr, backend_ctx.data_cache_buf, FLASH_SEC_SIZE);
	if (ret != 0) {
	...
	if (backend_ctx.error != 0) {
		LOG_ERR("Error in coredump backend (%d)!",	backend_ctx.error);
	}else{
		coredump_set_save_to_region_flag(true);
	}
	LOG_ERR(COREDUMP_PREFIX_STR COREDUMP_END_STR);
	//LOG_ERR("savelen=0x%x, ck=0x%x\n", backend_ctx.w_size, backend_ctx.checksum);
}
...
{
	int i;
	uint8_t *pcache = backend_ctx.data_cache_buf;
	uint32_t offset = backend_ctx.buf_offset;
	//add for logging dump
	coredump_logging_backend_buffer_output(buf, buflen);
	//LOG_ERR("off=0x%x, len=0x%x\n", offset, buflen);
	...
	for (i = 0; i < buflen; i++) {
		...
		pcache[offset] = buf[i];
		offset++;
		if(offset >= FLASH_SEC_SIZE) {
			backend_ctx.error = flash_write(backend_ctx.storage_dev,
						backend_ctx.base_addr+backend_ctx.cur_offset,
						pcache, FLASH_SEC_SIZE);
			if (backend_ctx.error != 0) {
				LOG_ERR("Error in  backend write (%d)!",backend_ctx.error);
				break;
			}
			backend_ctx.cur_offset += FLASH_SEC_SIZE;
			backend_ctx.w_size +=  FLASH_SEC_SIZE;
			offset = 0;
		}
	}
	backend_ctx.buf_offset = offset;
	//LOG_ERR("off=0x%x\n", offset);
}
...
struct z_coredump_backend_api z_coredump_backend_nand_sd = {
...
SYS_INIT(coredump_region_init, POST_KERNEL, 70);
