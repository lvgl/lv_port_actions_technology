...
void hx3605_hrv_ppg_init(void) //20200615 ericy ppg fs=25hz, phase3 conversion ready interupt en
{
	...
	uint8_t led_on_time =
		32; /* 0 = 32clk2=8us ; 8 = 64clk=16us; 16=128clk=32us ; 32 = 256clk=64us ;
...
void PPG_hrv_Calibration_Routine(HRV_CAL_SET_T *calR, int32_t led, int32_t amb)
{
	...
	case hrvLedCur:
		...
		if (led > amb + 128) {
			...
			calR->led_cur =
				10000 * (hx3605_hrs_agc_idac + calR->amb_idac) / calR->led_step;
			...
			calR->ontime = 32;
			calR->flag =
				CAL_FLG_LED_DR | CAL_FLG_LED_DAC | CAL_FLG_AMB_DAC | CAL_FLG_RF;
			calR->state = hrvCalLed;
		} else {
			calR->led_cur = calR->led_max_cur;
			calR->led_idac = 0;
	...
	AGC_LOG("AGC: led_drv=%d,ledDac=%d,ambDac=%d,ledstep=%d,ontime=%d,state=%d\r\n",
		calR->led_cur, calR->led_idac, calR->amb_idac, calR->led_step, calR->ontime,
		calR->state);
}
