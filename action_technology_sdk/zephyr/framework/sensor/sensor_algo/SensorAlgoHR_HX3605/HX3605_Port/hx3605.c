...
const uint8_t COUNT_BLOCK_NUM =
	50; //delay the block of some single good signal after a series of bad signal
const uint8_t SPO2_LOW_XCORR_THRE =
	30; //(64*xcorr)'s square below this threshold, means error signal
...
const uint16_t MEAN_NUM =
	32; //the length of smooth-average ;the value of MEAN_NUM can be given only 256 and 512
const uint8_t G_SENSOR = 0; //if =1, open the gsensor mode
const uint8_t SPO2_GSEN_POW_THRE = 150; //gsen pow judge move, range:0-200;
const uint32_t SPO2_BASE_LINE_INIT =
	135500; //spo2 baseline init, = 103000 + ratio(a variable quantity,depends on different cases)*SPO2_SLOPE
...
const uint16_t SPO2_LOW_CLIP_END_TIME =
	1500; //low clip mode before this data_cnt, normal clip mode after this
...
uint16_t hx3605_read_fifo_data(int32_t *buf, uint8_t phase_num, uint8_t sig)
{
	...
	for (ii = 0; ii < fifo_read_length; ii++) {
		if (sig == 0) {
			buf[ii] =
				(int32_t)(fifo_data_buf[ii * 3] | (fifo_data_buf[ii * 3 + 1] << 8) |
					  ((fifo_data_buf[ii * 3 + 2] & 0x1f) << 16));
		} else {
			if ((fifo_data_buf[ii * 3 + 2] & 0x10) != 0) {
				buf[ii] = (int32_t)(fifo_data_buf[ii * 3] |
						    (fifo_data_buf[ii * 3 + 1] << 8) |
						    ((fifo_data_buf[ii * 3 + 2] & 0x0f) << 16)) -
					  1048576;
			} else {
				buf[ii] = (int32_t)(fifo_data_buf[ii * 3] |
						    (fifo_data_buf[ii * 3 + 1] << 8) |
	...
	if (data_flg_end < phase_num - 1) {
		for (ii = fifo_out_length; ii < fifo_out_length + phase_num - data_flg_end - 1;
		     ii++)
			hx3605_brust_read_reg(0x31, databuf, 3);
...
bool hx3605_init(WORK_MODE_T mode)
{
	...
	case SPO2_MODE:
		printk("hx3605_init: SPO2_MODE\n");
#ifdef SPO2_DATA_CALI
		hx3605_spo2_data_cali_init();
#endif
#ifdef SPO2_ALG_LIB
		tyhx_spo2_para_usuallyadjust(SPO2_LOW_XCORR_THRE, SPO2_LOW_SNR_THRE,
					     COUNT_BLOCK_NUM, SPO2_BASE_LINE_INIT, SPO2_SLOPE,
					     SPO2_GSEN_POW_THRE);
		tyhx_spo2_para_barelychange(MEAN_NUM, SOP2_DEGLITCH_THRE, SPO2_REMOVE_JUMP_THRE,
					    SPO2_LOW_CLIP_END_TIME, SPO2_LOW_CLIP_DN,
					    SPO2_NORMAL_CLIP_DN, IR_AC_TOUCH_THRE, IR_FFT_POW_THRE,
					    SPO2_CALI, SLOPE_PARA_MAX, SLOPE_PARA_MIN);
		if (hx3605_spo2_enable() == SENSOR_OP_FAILED) {
			printk("hx3605_spo2_enable fail\n");
			return false;
		}
		printk("hx3605_init: SPO2_MODE done\n");
#endif
		break;

	case HRV_MODE:
#ifdef HRV_ALG_LIB
		printk("hx3605_init: HRV_MODE\n");
	...
	case SPO2_MODE: {
		...
		if (cal.work) {
			AGC_LOG("INT_AGC: Rled_drv=%d,Irled_drv=%d,RledDac=%d,IrledDac=%d,ambDac=%d,Rledstep=%d,Irledstep=%d,Rrf=%d,Irrf=%d,\r\n",
				cal.red_cur, cal.ir_cur, cal.red_idac, cal.ir_idac, cal.amb_idac,
				cal.red_led_step, cal.red_led_step, cal.ontime, cal.state);
		}
#endif
		break;
	}
...
{
	printk("ppg_timeout_handler,mode:%d\n", work_mode_flag);
	switch (work_mode_flag) {
	case HRS_MODE:
#ifdef HRS_ALG_LIB
...
void hx3605_hrs_ppg_Int_handle(void)
{
	...
	*count = 8;
	...
	if (hrs_wear_status == MSG_HRS_WEAR && (*count > 0)) {
		tyhx_hrs_alg_send_data(PPG_buf, *count, gsen_fifo_x_send, gsen_fifo_y_send,
				       gsen_fifo_z_send);
	}
#endif
	...
	if (hrs_wear_status) {
		...
		hr_os_api.wear_handler(1, true);
	} else {
		hr_os_api.hb_handler(0, 0, 0);
		hr_os_api.wear_handler(0, true);
	}
...
{
	printk("hx3605_spo2_ppg_Int_handle\n");
	uint8_t ii = 0;
	...
	*count = 8;
	...
	hx3605_spo2_alg_send_data(red_buf_vec, ir_buf_vec, green_buf_vec, *count, gsen_fifo_x,
				  gsen_fifo_y, gsen_fifo_z);
#else
	if (hx3605_spo2_read(&spo2_s_dat) == 0) {
		printk("hx3605_spo2_read fail\n");
		return;
	}
	for (ii = 0; ii < *count; ii++) {
	...
	if (spo2_wear_status == MSG_SPO2_WEAR && (*count > 0)) {
#ifdef GSENSER_DATA
		tyhx_spo2_alg_send_data(red_buf, ir_buf, cal.red_idac, cal.ir_idac, *count,
					gsen_fifo_x, gsen_fifo_y, gsen_fifo_z);
#else
		tyhx_spo2_alg_send_data(red_buf, ir_buf, cal.red_idac, cal.ir_idac, *count,
					gen_dummy, gen_dummy, gen_dummy);
#endif
	}
#endif
	...
	if (spo2_wear_status == MSG_SPO2_WEAR) {
		hr_os_api.spo2_handler(alg_results.spo2_result, 0, alg_results.hr_result, 0, 0, 0,
				       0, 0);
		hr_os_api.wear_handler(1, true);
	} else {
		hr_os_api.spo2_handler(0, 0, 0, 0, 0, 0, 0, 0);
		hr_os_api.wear_handler(0, true);
	}

	printk("spo2_wear_status:%d, spo2:%d \r\n", spo2_wear_status, alg_results.spo2_result);
/////////////////////////////////////////////////////////////////////////////////////
#ifdef TYHX_DEMO
	oled_dis.refresh_time++;
...
void hx3605_hrv_ppg_Int_handle(void)
{
...
#else
	if (hx3605_hrv_read(&hrv_s_dat) == 0) {
		return;
	}
	for (ii = 0; ii < *count; ii++) {
	...
	if (wear_mode == MSG_HRV_WEAR && (*count > 0 && *count <= 32)) {
		alg_hrv_results = tyhx_hrv_alg_send_bufdata(PPG_buf, *count, 0, 0, 0);
	}
#endif

	if (wear_mode == MSG_HRV_WEAR) {
		hr_os_api.hrv_handler(0, 0, alg_hrv_results.hrv_result);
		hr_os_api.wear_handler(1, true);
	} else {
		hr_os_api.hrv_handler(0, 0, 0);
		hr_os_api.wear_handler(0, true);
	}
	printk("alg_hrv_results,wear:%d,hrv:%d, status:%d, cnt:%d \r\n", wear_mode,
	       alg_hrv_results.hrv_result, alg_hrv_results.hrv_alg_status,
	       alg_hrv_results.data_cnt);
...
void hx3605_wear_ppg_Int_handle(void)
{
	...
	if (hr_os_api.wear_handler) {
		printk("hx3605_wear_ppg_Int_handle:%d\n", hx3605_check_mode_status);
		hr_os_api.wear_handler(hx3605_check_mode_status, false);
	}
