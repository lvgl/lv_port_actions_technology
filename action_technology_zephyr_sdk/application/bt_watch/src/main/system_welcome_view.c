...
static int _welcome_view_layout(view_data_t *view_data)
{
	...
	lv_anim_set_duration(&anim, CONFIG_WELCOME_ANIM_TIME);
...
int _welcome_view_handler(uint16_t view_id, view_data_t *view_data, uint8_t msg_id, void *msg_data)
{
	switch (msg_id) {
...
VIEW_DEFINE2(welcome_view, _welcome_view_handler, NULL, NULL, WELCOME_VIEW,
