 ...
 */
int mbedtls_aes_crypt_ecb_sw( mbedtls_aes_context *ctx,
                           int mode,
                           const unsigned char input[16],
                           unsigned char output[16] )
{
                           ...
                           unsigned char *output )
{
    ...
    /* Compute the tweak. */
    ret = mbedtls_aes_crypt_ecb_sw( &ctx->tweak, MBEDTLS_AES_ENCRYPT,
                                 data_unit, tweak );
    if( ret != 0 )
        return( ret );
    ...
    while( blocks-- )
    {
        ...
        ret = mbedtls_aes_crypt_ecb_sw( &ctx->crypt, mode, tmp, tmp );
        if( ret != 0 )
            return( ret );
                       ...
                       unsigned char *output )
{
    ...
    if( mode == MBEDTLS_AES_DECRYPT )
    {
        while( length-- )
        {
            if( n == 0 )
            {
                ret = mbedtls_aes_crypt_ecb_sw( ctx, MBEDTLS_AES_ENCRYPT, iv, iv );
                if( ret != 0 )
                    goto exit;
            }
