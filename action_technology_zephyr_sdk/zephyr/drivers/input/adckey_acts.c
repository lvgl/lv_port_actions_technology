...
out:
	...
	if (cfg->adc_chan != PMUADC_ID_LRADC1) {
		// Attempt to submit a delayed work item for execution.
		// If the submission fails, enter a loop to retry.
		while(k_delayed_work_submit(&adckey->timer, K_MSEC(cfg->poll_interval_ms)) != 0){
			try ++;

			// If the maximum number of attempts is exceeded, call the k_panic function.
			// The k_panic function causes the system to enter a panic state, typically due to an irrecoverable error.
			if(try > 3)
				k_panic();
			}
	} else {
...
static void adckey_acts_enable(const struct device *dev)
{
	...
	k_timer_start(&adckey->timer, K_MSEC(cfg->poll_interval_ms), K_NO_WAIT);
#else
	if(k_delayed_work_submit(&adckey->timer, K_NO_WAIT) == 0){
		adckey->state = 1;
	}
	else{
		// If the work item submission fails, register PMU notification.
		if (soc_pmu_register_notify(&detect_param))
				LOG_ERR("failed to register pmu notify");
	}
#endif
