		...
		uint32_t cmd, void *buf, uint32_t len)
{
	...
	/* 2) Swith cpuclk source to HOSC */
	uint32_t sysclk_bak = sys_read32(CMU_SYSCLK);
	sys_write32((sysclk_bak & (~0x7)) | 0x1, CMU_SYSCLK);
	k_busy_wait(1); // delay 1us
	/* 3) cpu clk select HOSC,  ahb div 4 */
	sys_write32(0x101, CMU_SYSCLK);
