...
#define HRTIMER_TIMEOUT_MS (3000)
...
static void _hrtimer_reload(timer_register_t *timer_register, uint32_t value)
{
    ...
    TT_DEBUG("CTL=0x%x, VAL=0x%x, CNT=0x%x\n", timer_register->ctl,
        timer_register->val, timer_register->cnt);
}

static uint32_t _hrtimer_get_jiffies(timer_register_t *timer_register, uint32_t load_val)
{
    uint32_t val;

    if (soc_in_sleep_mode()) {
        load_val = load_val / MUTIPLE;
    }

    /*hw timer bug, we should check*/
    if (load_val > timer_register->cnt) {
        val = load_val - timer_register->cnt;
    } else {
        val = 0;
    }
...
static uint64_t _get_current_jiffies(void)
{
    ...
    val = _hrtimer_get_jiffies(ctx->timer_register, ctx->timer_load_val);
...
void hrtimer_handle_expired(void)
{
    ...
    SYS_DLIST_FOR_EACH_CONTAINER_SAFE(&ctx->hrtimer_q, cur, tmpNode, node){
        ...
        sys_dlist_append(&call_and_del, &cur->node_fn);

        // check timeout
        cur_time = (uint32_t)soc_sys_uptime_get();
        __ASSERT((cur_time - pre_time) <= HRTIMER_TIMEOUT_MS, "");
    }
    ...
    SYS_DLIST_FOR_EACH_CONTAINER_SAFE(&call_and_del, cur, tmpNode, node_fn){
		// timer is expired and start to excute timer function
        sys_dlist_remove(&cur->node_fn);
...
static void hrtimer_runtime_suspend(void)
{
	...
	// limit period at least 2ms to fix resume issue
	if (ctx->timer_load_val < (CONFIG_HOSC_CLK_MHZ * 1000) * 2) {
		ctx->timer_load_val = (CONFIG_HOSC_CLK_MHZ * 1000) * 2;
	}
    ...
    sys_write32(S3_SOURCE, CMU_TIMER1CLK); //select RC32K
