...
static void free_list_remove_bidx(struct z_heap *h, chunkid_t c, int bidx)
{
	...
	if (next_free_chunk(h, c) == c) {
		...
		CHECK((first >= right_chunk(h, 0)) && (first < h->end_chunk));
		CHECK((second >= right_chunk(h, 0)) && (second < h->end_chunk));
...
/* update used and peak used */
static void update_usage(struct z_heap *h, chunkid_t c, bool used)
{
	struct z_heap_stats *stats = heap_get_stats(h);
	chunksz_t size = chunk_size(h, c);

	if (used) {
		stats->used_chunks += size;
		if (stats->used_chunks > stats->peak_chunks) {
			stats->peak_chunks = stats->used_chunks;
		}
	} else {
		stats->used_chunks -= size;
	}
...
void sys_heap_free(struct sys_heap *heap, void *mem)
{
	...
	if (tracedump_get_enable(TRACE_HEAP_LEAK)) {
		tracedump_remove(TRACE_HEAP, (uint32_t)mem);
	} else {
		tracedump_save(TRACE_HEAP, (uint32_t)mem, 0);
	}
...
static chunkid_t alloc_chunk(struct z_heap *h, chunksz_t sz)
{
	...
	if (b->next) {
		...
		do {
			...
			CHECK((b->next >= right_chunk(h, 0)) && (b->next < h->end_chunk));
...
void *sys_heap_alloc(struct sys_heap *heap, size_t bytes)
{
	...
	update_usage(h, c, true);
	mem = chunk_mem(h, c);

	tracedump_save(TRACE_HEAP, (uint32_t)mem, bytes);
...
void *sys_heap_aligned_alloc(struct sys_heap *heap, size_t align, size_t bytes)
{
	...
	if (align != rew) {
		...
		if (align <= chunk_header_bytes(h)) {
			tracedump_lock();
			mem =  sys_heap_alloc(heap, bytes);
			tracedump_unlock();
			goto algined_alloc_ret;
		}
		rew = 0;
			       ...
			       size_t align, size_t bytes)
{
	...
	if (ptr == NULL) {
		tracedump_lock();
		ptr = sys_heap_aligned_alloc(heap, align, bytes);
		tracedump_unlock();
		goto aligned_realloc_ret;
	}
	if (bytes == 0) {
	...
	if (tracedump_get_enable(TRACE_HEAP_LEAK)) {
		tracedump_remove(TRACE_HEAP, (uint32_t)ptr);
	}
...
aligned_realloc_ret:
	tracedump_save(TRACE_HEAP, (uint32_t)ptr, bytes);
...
void sys_heap_init(struct sys_heap *heap, void *mem, size_t bytes)
{
	...
	/* init stats */
	struct z_heap_stats *stats = heap_get_stats(h);
	stats->used_chunks = chunk0_size + heap_footer_bytes(bytes);
	stats->peak_chunks = stats->used_chunks;
}
