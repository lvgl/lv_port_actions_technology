...
#else
    static void draw_letter_acts_outline(lv_draw_vg_lite_unit_t * u, const lv_draw_glyph_dsc_t * dsc);
...
#else
            case LV_FONT_GLYPH_FORMAT_VECTOR: {
                    draw_letter_acts_outline(u, glyph_draw_dsc);
                }
                break;
#endif /* LV_USE_FREETYPE */
...
static void draw_letter_acts_outline(lv_draw_vg_lite_unit_t * u, const lv_draw_glyph_dsc_t * dsc)
{
    if(!lv_area_is_on(u->base_unit.clip_area, dsc->letter_coords)) {
        return;
    }
    ...
    /* convert to vg-lite coordinate */
    vg_lite_translate(pos.x - dsc->g->ofs_x, pos.y + dsc->g->box_h + dsc->g->ofs_y, &draw_matrix);
    vg_lite_scale(outline->scale, outline->scale, &draw_matrix);

    vg_lite_float_t box_x1 = dsc->g->ofs_x / outline->scale;
    vg_lite_float_t box_x2 = (float)(dsc->g->ofs_x + (int16_t)dsc->g->box_w) / outline->scale;
    vg_lite_float_t box_y1 = (float)(-dsc->g->ofs_y - (int16_t)dsc->g->box_h) / outline->scale;
    vg_lite_float_t box_y2 = (float)(-dsc->g->ofs_y) / outline->scale;

    vg_lite_path_t path;
    vg_lite_init_path(&path, VG_LITE_S16, VG_LITE_HIGH, outline->path_len,
            outline->path_data, box_x1, box_y1, box_x2, box_y2);
